...
computed by `ai/skills/universal/retro/assets/dev-flow-stats.sh` ([#663](https://github.com/evanharmon1/harmon-devkit/issues/663))
...
- There are **no `[tier.*]` tables**. Model-stratum classification belongs to
  ...
  generated by `agent-registry-labels.mjs tier-roles` from the registry's
  `roles[]` and the shared `TIER_ORDER` ladder in `registry-roles.mjs`
  (pinned equal to the policy's `tier_order` by the invariant in
  `devflow-policy.mjs`) into `label-registry.json` via the same
  single-source path as the existing `suggest:*`, `claim:*`, and
  `foreman:<adapter>` labels.
