...
| [`universal/implement`](./ai/skills/universal/implement/)                     | Ready       | `/implement` — drive a claimed issue to an open PR: read it as a spec, work the repo's dev loop (check → verify → second-model review → CI mirror), tick acceptance criteria as verified, open the PR. Never claims, never merges                                          |
