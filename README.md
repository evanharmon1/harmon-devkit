...
[![Clone in Local Dev Container](https://img.shields.io/static/v1?label=Local%20Dev%20Container&message=Clone&color=blue&logo=visualstudiocode)](https://vscode.dev/redirect?url=vscode%3A//ms-vscode-remote.remote-containers/cloneInVolume%3Furl%3Dhttps%3A//github.com/evanharmon1/harmon-devkit)
...
| [`claude-hooks/session-end-archive`](./templates/claude-hooks/session-end-archive/)                            | AI         | Claude Code SessionEnd hook that gzips every session transcript into an archive dir before the `cleanupPeriodDays` retention deletes it (script + settings snippet)     |
...
| [`frontend/implement-design`](./ai/skills/frontend/implement-design/)         | Ready       | Implements a Claude Design `.tar.gz` handoff into a real codebase — reconcile an existing design system or bootstrap a new one (tokens → shadcn/Tailwind v4 OKLCH, `/brand`, contrast + licensing gates) |
...
| [`universal/kickoff`](./ai/skills/universal/kickoff/)                         | Ready       | `/kickoff` — start-of-session ritual: get oriented in the repo and compose a descriptive session name (emits a copy-pasteable `/rename`)                                                                                                                                  |
| [`universal/breakdown`](./ai/skills/universal/breakdown/)                     | Ready       | `/breakdown` — decompose a lump of work (feature, epic, strategy doc, big issue) into GitHub issues sized for one session / one PR / one human review, with milestones and sub-issues where warranted, explicit blocked-by dependency edges, and the target repo's own labels — proposing the full plan for one human approval before any GitHub write |
| [`universal/claim`](./ai/skills/universal/claim/)                             | Ready       | `/claim` — pre-implementation sanity check: verify the target issue and related PRs against the live repo, then claim the issue                                                                                                                                           |
| [`universal/implement`](./ai/skills/universal/implement/)                     | Ready       | `/implement` — drive a claimed issue to a draft PR ready for the integration stage: read it as a spec, work the repo's dev loop (check → verify → second-model review → CI mirror), tick acceptance criteria as verified, open the PR with its deferred findings recorded, then stop; the same session continues into the integration stage (`/integrate`) from there, stopping at ready-for-review. Never claims, never merges |
| [`universal/review`](./ai/skills/universal/review/)                           | Ready       | `/review` — drive one challenge or verification-review confidence stage through authenticated passes, adjudication, durable rendering, and bounded convergence                                                                                                               |
| [`universal/orchestrator`](./ai/skills/universal/orchestrator/)               | Ready       | `/orchestrator` — coordinate policy-resolved, worktree-isolated dev-flow runs, including scoped lanes, durable run records, assembly, and terminal-event monitoring                                                                                                         |
| [`universal/retro`](./ai/skills/universal/retro/)                             | Ready       | `/retro` — end-of-session retro: starts from the run's retained Dev flow v2 evidence (per-stage rounds vs caps, findings by class and provenance, overrides, interventions) where one exists, then loose ends, follow-ups, improvement opportunities, and linked status tables for every PR/issue touched                                                                                                                                   |
| [`universal/wrap`](./ai/skills/universal/wrap/)                               | Ready       | `/wrap` — end-of-session ritual: wrap up dangling work and emit the `/rename done-<name>` command                                                                                                                                                                       |
| [`universal/integrate`](./ai/skills/universal/integrate/)                     | Ready       | `/integrate` — integrate a draft PR to ready for review: dispatch the integrator agent for CI settlement and the Codex cloud-review cycle, adjudicate findings as hypotheses, fix confirmed ones with per-thread replies, push, re-dispatch (bounded by the resolved integration and remediation caps, or the repo's own; never merges) |
| [`universal/triage`](./ai/skills/universal/triage/)                           | Ready       | `/triage` — manifest-governed backlog classifier, tuned for cheap headless models (`task triage`): applies area/layer/domain and needs-triage labels, plus work-type labels on personal repos or native Issue Types on organization repos, through contract-enforcing scripts (dry-run by default), and upserts everything else into one rolling report issue |
| [`universal/track-work`](./ai/skills/universal/track-work/)                   | Ready       | GitHub issue/PR tracking hygiene — fires automatically when writing a PR body that closes an issue, filing a follow-up, citing an issue, or closing one. Ships executable checks (`Closes` vs `Refs`, unticked items, perishable claims, and a pre-create issue-authoring gate validating the title, canonical body skeleton, and proposed labels against the repo's label registry) that CI runs too, plus a narrowed ticking command for criteria you verify as you work |
...
For planning changes, active work lives in `openspec/changes/<change-name>/`.
Its capability delta specs are the implementation-ready contract for that
change; the root `specs/` documents are milestone-level anchors rather than the
active delta surface. Run `task openspec:validate` for strict change validation
and `task validate:openspec-skills` to guard the vendored OpenSpec workflow
skills, including their frontmatter, pinned wrapper, and declared permissions.
