...
| [`universal/triage`](./ai/skills/universal/triage/)                           | Ready       | `/triage` — manifest-governed backlog classifier, tuned for cheap headless models (`task triage`): applies area/layer/domain, work-type (personal repos only), and needs-triage labels through contract-enforcing scripts (dry-run by default), and upserts everything else into one rolling report issue |
