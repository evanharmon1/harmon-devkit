...
| [`universal/gauntlet`](./ai/skills/universal/gauntlet/)                       | Ready       | `/gauntlet` — run the second-model review gauntlet: adversarial challenge then verification review, each to convergence under its own resolved cap, then the CI mirror and the draft PR. Convergence is the exit, fixing findings is not; carries the damper catalog that keeps a self-reinforcing critique loop from feeding on itself |
