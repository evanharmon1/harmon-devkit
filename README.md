...
| [`universal/implement`](./ai/skills/universal/implement/)                     | Ready       | `/implement` — drive a claimed issue to a ready-for-review PR: read it as a spec, work the repo's dev loop (check → verify → second-model review → CI mirror), tick acceptance criteria as verified, open the PR, then continue through the integration stage. Never claims, never merges |
