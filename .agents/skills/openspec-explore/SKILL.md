...
```bash
"$(git rev-parse --show-toplevel)/scripts/openspec.sh" list --json
```
...
1. **Resolve and read existing artifacts for context**
   - Run `"$(git rev-parse --show-toplevel)/scripts/openspec.sh" status --change "<name>" --json`.
...
- **Don't auto-capture** - Offer to save insights, don't just do it. Read-only commands and tools need no confirmation. Before the first write-capable action—including `"$(git rev-parse --show-toplevel)/scripts/openspec.sh" new change` or another command that writes files—name the artifacts or files and proposed changes, ask a direct yes/no question, and wait for explicit confirmation in a separate user message. That confirmation covers only the described scope; ask again before expanding it. Answers to design or clarifying questions are never consent to write.
- **Don't manually scaffold changes** - Never create a new change directory under `openspec/changes/` by hand. Always use `"$(git rev-parse --show-toplevel)/scripts/openspec.sh" new change "<name>"` (with `--store <id>` when applicable) so required metadata such as `.openspec.yaml` is created before writing artifacts.
