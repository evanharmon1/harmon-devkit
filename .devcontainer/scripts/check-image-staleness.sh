...
while IFS= read -r f; do
    ...
    if printf '%s' "${names}" | grep -qxF "      ${rel}"; then # shell-robustness: ok — pre-existing; the fail-open risk is tracked in #822 and deliberately not fixed in a test-robustness PR
        continue
    fi
