...
- Wave 3b (after 6.1): 6.3, then 5.1 once 6.2 and 6.3 have merged.
  harmon-init#1080 (the pre-PR gate the anchor sequences before #604) is
  already closed, so it needs no lane.
...
- [ ] 6.3 Complete [#711](https://github.com/evanharmon1/harmon-devkit/issues/711) after 6.1 (a DevKit PR applying the Harmon Init copier update): migrate `.devflow.toml` to `schema_version = 2` with the reader from 2.3 already at the merge base, then sweep the other generated repositories the same way; verify `scripts/devflow-policy.mjs detect` reports v2 on `main`, `task verify` passes, and no v2 consumer refuses the live policy.
