...
- Wave 1 (concurrent lanes, ordered merge): 2.1, 2.3, and 2.4 start
  ...
  is 1.3, 1.1, 2.1, 2.4, 2.3, 2.5. Because 2.1 and 1.3 both extend the
  single-document schema harness, 2.3 validates challenger passes against
  2.1's schema, 2.3 and 2.4 consume the validator 1.3 finishes, and 2.5
  extends the registry schema 2.1 ships with the trusted-orchestrator
  allowlist, each of 2.1, 2.3, 2.4, and 2.5 merges the updated `main`
  (never a rebase: pushed history is not rewritten) once every task ahead
  of it in that order has merged, and re-runs its full suite (for 2.1, the
  complete schema suite) before it leaves draft; until then its PR body
  records the seam as an unsettled deferred finding. 1.2 follows 1.1
  (maintainer ticks).
- Wave 2a (after wave 1, and specifically after 2.5 for 4.1 — its evidence
  trust-root authentication has no allowlist to validate against
  otherwise): 2.2, 3.2, and 4.1.
- Wave 2b (after 2.2 and 3.2, since `/orchestrate` owns the integration
...
**Runtime paths below were relocated by
[#974](https://github.com/evanharmon1/harmon-devkit/issues/974).** The v2
runtime scripts these tasks name (`devflow-policy.mjs`,
`validate-result-schemas.mjs`, `render-dev-flow.*`, `dev-flow-exit.*`,
`round-push.sh`, `normalize-finder-findings.mjs`, `dev-flow-monitor.sh`,
`consumer-pin-audit.sh`, `dev-flow-stats.mjs`) were authored under the
repository root `scripts/` and now live with the skills that invoke them —
shared ones in `ai/skills/universal/dev-flow-support/assets/`, single-owner
ones in the owning skill's own `assets/`. The skills sync is their only
distribution channel; nothing ships through the harmon-init template. Task
prose below names the CURRENT path, so a reader can follow it to a file that
exists; the task's original wording named the root path it was written
against.
...
- [ ] 1.3 Complete [#686](https://github.com/evanharmon1/harmon-devkit/issues/686): implement the single-document schema and validator residue from PR #678, including clean-verdict, native-envelope, digest, path, and managed-set cases; verify `./ai/skills/universal/dev-flow-support/assets/test-result-schemas.sh` passes.

## 2. Establish registry, gates, computation, and rendering

- [ ] 2.1 Complete [#635](https://github.com/evanharmon1/harmon-devkit/issues/635): add registry roles, challenger/reviewer separation, finders, exact multi-model tier defaults, harness role support, write boundaries, execution-label provenance, and cross-file validation, and ship `ai/schemas/result.challenger.schema.json` with its fixture corpus so every registry role names an existing result schema (moved here from #638 because the exit script validates challenger passes); verify missing/duplicate-default, missing-schema, and interactive/unattended label-provenance cases plus `task validate:agents` and `task test:result-schemas` pass, then post the registry tier and default vocabulary on harmon-init#1081. **Note:** #635 closed without adding the evidence delta spec's trusted-orchestrator allowlist requirement — tracked separately as [#741](https://github.com/evanharmon1/harmon-devkit/issues/741) below, not reopened here.
- [ ] 2.3 Complete [#636](https://github.com/evanharmon1/harmon-devkit/issues/636): ship the shared v2 policy reader (`ai/skills/universal/dev-flow-support/assets/devflow-policy.mjs`: shape detection, legacy/v1 refusal with migration messages, rigor/rounds/breadth/gates/convergence/role/stage resolution, and the merge-base rule) with its `devflow:policy` Taskfile target (resolve and detect modes, tested) and `ai/skills/universal/dev-flow-support/assets/dev-flow-exit.sh` (the retired `gauntlet-exit.sh` name is not used), implementing trajectory receipt, chronology and provenance/fingerprint verification, logical finder rounds, and deterministic `continue|converged|diverging|capped` computation from `[rounds]` and `[convergence]`, with the predicate catalog from `specs/dev-flow-v2.md` pinned by name; verify every outcome, invalidation boundary, shape-refusal, and omator replay fixture passes against fixture policies only (never this repository's live `.devflow.toml`), plus the legacy-to-v2 and v1-to-v2 merge-base decoder fixtures that (a) mutate every protected value in the branch policy copy, in the branch reader's defaults, and in the branch `agent-registry.json` (roles, write boundaries, trusted actor IDs, tiers) and prove the resolved policy is unchanged, and (b) — the fixture (a) alone cannot force, since a decoder that ignores the merge-base's own declared values and always returns built-ins would still pass it — separately assert that when the **merge-base** copy itself declares a non-default tier, breadth, or strategy, the decoded result equals that declared value rather than silently falling back to a built-in, with the legacy shared `shepherd` budget preserved, then post the pinned predicate names and composition grammar on harmon-init#1081.
- [ ] 2.2 Complete [#632](https://github.com/evanharmon1/harmon-devkit/issues/632) after 2.3: move the round-push broker to the repository-owned path `ai/skills/universal/review/assets/round-push.sh` (so `git show <merge-base>:ai/skills/universal/review/assets/round-push.sh` can materialize it outside the worktree; for this relocation change alone the merge-base broker is the skill-asset copy it relocates, tested as such; the scanner closure is `scripts/gitleaks-scan.sh`, `.gitleaks.toml`, `scripts/summarize-gitleaks.mjs`, and `.gitleaksignore` if the merge base has one, invoked by explicit path from the extracted tree rather than through the worktree's `security:secrets` recipe), leave the existing gauntlet asset untouched and legacy-only (it keeps serving the still-shipped gauntlet procedure against this repository's legacy policy until 6.3, and is deleted with the skill in 5.1; the successor skills reference the new path, and no shim delegates one to the other), make it resolve bare gate targets and the sole docs allowlist through the policy reader, re-derive diff class, always scan secrets, and record branch-resident local gate results as branch-attested rather than CI-authoritative; verify docs-only, code, mixed-diff, and required-check-substitution refusal fixtures, a dependency-only closure fixture (a branch that changes only the policy reader, the scanner configuration, the scanner's summary helper, or the `security:secrets` Taskfile recipe still gates with the merge-base copies), and the relevant skill tests pass, with `scripts/test-gauntlet-push.sh` still green against the untouched asset.
- [ ] 2.4 Complete [#637](https://github.com/evanharmon1/harmon-devkit/issues/637): render deferred findings, adjudications, policy disclosure, round tables, blockers, and thread plans from one validated record with marked-section ownership and latest-read handoff, scanning every rendered projection for secrets and posting a redacted `[REDACTED:<rule-id>]` projection in place of a detected span while the unredacted evidence stays local (the evidence delta spec's own "Evidence is scanned and safely redacted" requirement); document GitHub's unconditional-write race, and verify golden outputs, post-write mismatch repair, bounded-retry blockers, concurrent human body edits, fresh-read re-merges, secret-redaction, and interruption/idempotency fixtures pass.
- [ ] 2.5 Complete [#741](https://github.com/evanharmon1/harmon-devkit/issues/741) after 2.1: add a top-level trusted-orchestrator actor-ID allowlist to `agent-registry.json`/its schema (distinct from each finder's own `trusted_actor_id`) — the evidence delta spec's run-record and evidence-comment authentication root — and pin trust evaluation for a historical run to an authoritative registry revision rather than the registry's current content; verify missing/empty-allowlist (fail closed), an actor added after evidence was posted (does not retroactively authenticate it), and an actor removed after posting (does not invalidate already-authenticated historical evidence) fixtures pass.

## 3. Replace stage orchestration and prove trajectory invariants

- [ ] 3.1 Complete [#638](https://github.com/evanharmon1/harmon-devkit/issues/638) after 2.1, 2.3, 2.4, and 2.2: add challenger/reviewer agents, the `/review` stage skill, and `/orchestrate` standing mode with scoped dispatch, parallel-implementer bounds, single-writer lane assembly, reserve-first external actions, postcondition-reconciled persistent monitoring, and merge scheduling, consuming the challenger schema from 2.1, the policy reader and exit script from 2.3, the renderer from 2.4, and the broker from 2.2 by path (the skill references `ai/skills/universal/review/assets/round-push.sh` and does not edit or vendor it), and repointing `/implement`'s hand-offs from the retired `gauntlet` and `shepherd` names to `/review` and `/integrate` so the successor skills are reachable before the live policy migrates in 6.3; verify crash-after-write adoption, absent-action retry, indeterminate-postcondition refusal, skill/agent validation, and the fixture-driven stage dry run pass.
...
- [ ] 4.1 Complete [#663](https://github.com/evanharmon1/harmon-devkit/issues/663): implement read-only run harvesting, append-only digest-chained history, closed-cohort unattended-success metrics, per-run JSON/table output, immutable `--as-of` reconstruction, and convergence-policy replay; verify cutoff reconstruction, single-writer replay stability (a byte-identical duplicate transition/intervention/outcome entry collapses to one; a forked chain — two entries claiming the same previous-entry digest with different content — reports the run indeterminate rather than choosing a branch; a fixed cutoff reconstructs identically across repeated reads of the same durably-observed chain, though a later read can newly see an entry that lands after an earlier read completed), evidence trust-root authentication (configured trusted-orchestrator actor IDs from `agent-registry.json` only — never a kickoff-shaped event alone, and never an identity declared inside the record), edited/deleted-entry rejection, and omator/Foreman trajectory fixtures pass.
...
- [ ] 6.1 Complete [harmon-init#1081](https://github.com/evanharmon1/harmon-init/issues/1081) after 2.1 and 2.3: render the `schema_version = 2` `.devflow.toml` template with `[rounds.*]`, `[breadth.*]`, `[gates]`, the composed `[convergence]` catalog pinned by 2.3, `[role.*]`, `[stage.*]`, and the `cursory`–`forensic` ladder, remove `[tier.*]`, and validate every slug against the registry shipped by 2.1; verify `scripts/test-devflow-config.sh` and the copier render pass, and that the rendered `schema_version = 2` file ships a `.devflow-conformance-v2.json` fixture corpus per ADR 0008's versioning pattern.
- [ ] 6.2 Complete [harmon-init#1082](https://github.com/evanharmon1/harmon-init/issues/1082) after 3.1 and 3.2: shrink the AGENTS.md Dev Loop to policy invariants plus a pointer to `/implement`, `/review`, `/integrate`, and `/orchestrate`; verify the section is under 150 lines and `audit:agent-instructions` passes.
