...
1. Write the spec using the template below, then publish it to the project
   issue tracker. Determine and apply exactly one category role (`bug` or
   `enhancement`) from the problem being specified, plus the
   `ready-for-agent` state role — no need for additional triage.
