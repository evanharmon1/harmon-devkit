...
# followed by " if ... %]") plus whitespace — the case the repo's shell
# formatter helper must survive.
jinja_open='[%'
...
test_deps="$(yq -r '.tasks.test.deps[]' Taskfile.yml | sort)"
...
fixture_deps="$(yq -r '.tasks.test.deps[]' "$dedupe_fixture" | sort)"
