...
GitHub **UI import** — do this only once `build.yml`, `devcontainer-build.yml` are on
...
**Why the UI, not `gh api … rulesets`:** a blind REST `POST` is **not
idempotent** (every run creates another "Protect Main" ruleset — silent
duplicates). The
...
**Read is cheap; write is the line.** Variables are read-only above for a
reason that is not squeamishness — see the exclusions below.
...
```json
{
  ...
  "source": "evanharmon1/harmon-devkit",
  "enforcement": "active",
  "conditions": {
    "ref_name": {
