...
| finder | An external review product that produces findings (`codex-adversarial`, `codex-verification`, `codex-cloud`; `coderabbit`/`copilot` are illustrative, not yet registered), declared in the registry with its GitHub actor id and stage affinity. Distinct from a **harness** (`codex-cli`, `claude-code`, `antigravity`): a harness runs an agent in a role, a finder is a review product registered against a stage. |
