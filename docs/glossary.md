...
| integration cap | (`integration` in `.devflow.toml`; formerly `shepherd`.) Bounds Codex re-review cycles only. Answering every human and CI finding is unconditional — never skipped — but the fix pushes it takes are bounded by the `remediation` cap, whose terminal action is escalation with the findings listed. `integration = 0` means "no Codex cycle required", never "abandon reviews". |
