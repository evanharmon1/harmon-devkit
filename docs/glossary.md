...
| finder | An external review product that produces findings (`codex-adversarial`, `codex-verification`, `codex-cloud`, `coderabbit-cloud`, `copilot-adversarial`, `copilot-verification`, `copilot-cloud`), declared in the registry with its stage affinity, its raw output shape, its severity map onto P0-P3 and, for a remote/cloud finder that posts its own comments, a trusted GitHub actor id plus the trigger mechanism and terminal-result signals that make one of its results terminal (a local-CLI finder like `codex-adversarial`/`codex-verification` needs none of that — its output is consumed directly, never authenticated by comment authorship). Codex is the shipped default; the others are opt-in and add no account or paid dependency until a maintainer enables them. Distinct from a **harness** (`codex-cli`, `claude-code`, `antigravity`): a harness runs an agent in a role, a finder is a review product registered against a stage. |
