...
| finder | An external review product that produces findings (`codex-adversarial`, `codex-verification`, `codex-cloud`; `coderabbit`/`copilot` are illustrative, not yet registered), declared in the registry with its stage affinity and, for a remote/cloud finder that posts its own comments, a trusted GitHub actor id (a local-CLI finder like `codex-adversarial`/`codex-verification` needs none — its output is consumed directly, never authenticated by comment authorship). Distinct from a **harness** (`codex-cli`, `claude-code`, `antigravity`): a harness runs an agent in a role, a finder is a review product registered against a stage. |
