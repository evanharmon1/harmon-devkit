...
printf '%s' "$command" | grep -qE '\bgit\b' || exit 0 # shell-robustness: ok — pre-existing; the fail-open risk is tracked in #822 and deliberately not fixed in a test-robustness PR
